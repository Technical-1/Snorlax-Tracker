  ...
  return (
    <div onClick={() => onOpen(card)}
      ...
      style={{ display:"flex",alignItems:"center",gap:9,padding:"7px 10px",borderRadius:8,cursor:"pointer",transition:"all 0.12s",
        background:isOwned?"rgba(74,222,128,0.07)":hover?"rgba(255,255,255,0.06)":"rgba(255,255,255,0.03)",
      ...
      <button aria-label={isOwned ? "Owned" : "Mark owned"}
        ...
        style={{width:19,height:19,borderRadius:5,cursor:"pointer",flexShrink:0,padding:0,lineHeight:1,
          border:`2px solid ${isOwned?"#4ade80":"rgba(255,255,255,0.22)"}`,
          background:isOwned?"#4ade80":"transparent",display:"flex",alignItems:"center",justifyContent:"center",
          fontSize:10,fontWeight:900,color:"#000",transition:"all 0.12s"}}>{isOwned?"✓":""}</button>
      <div style={{flex:1,minWidth:0}}>
        ...
        {card.imgManual && card.img && (
          <div style={{fontSize:7,fontWeight:700,color:"#818cf8"}}>manual image</div>
        )}
      ...
      <div style={{textAlign:"right",flexShrink:0}}>
        {card.market != null && <div style={{fontSize:11,fontWeight:700,color:card.priceManual?"#818cf8":"#fbbf24"}} title={card.priceManual?"your manual price":"market price"}>{fmtPrice(card.market)}{card.priceManual?" ✎":""}</div>}
