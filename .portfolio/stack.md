...
- **Hosting**: Static build (`npm run build` → `dist/`); deployable to any static host.
- **CI/CD**: None configured; verification is local (`typecheck` + `test` + `build`).
- **Monitoring**: None (no server).
